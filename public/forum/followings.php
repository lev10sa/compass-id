    ...
    if (isset($_GET['acc_main']) && isset($_GET['acc_sec']) && isset($_GET['type'])) {
        ...
        if (isset($_SESSION['s_em']) && isset($_SESSION['s_pw'])) {
            ...
            if ($_GET['type'] == 'follow') {
                ...
                $con->query($flSQL);
                header('location: ./followings.php?un=' . $un);
...
?>
    ...
    <head>
        ...
        <link rel="stylesheet" href="https://forum.compasspubindonesia.com/followers.css">
        <link
            rel="stylesheet"
    ...
    <body>
        ...
        <div class="body">
            ...
            <div class="mid">
                <div class="tabs">
                    <a href="./followers.php?un=<?= $pref['username']; ?>">Pengikut</a>
                    <a href="./followings.php?un=<?= $pref['username']; ?>" class="active">Mengikuti</a>
                    <a></a>
                </div>
                <div class="flower">
                    <?php
                    $sqlArt = "SELECT accounts.username, accounts.name, accounts.email, accounts.profile_pic, accounts.id, follows.acc_main, follows.acc_sec FROM accounts, follows WHERE accounts.id = follows.acc_sec AND follows.acc_main = '$id'";
                    $selArt = $con->query($sqlArt);
                    $selArt->setFetchMode(PDO::FETCH_ASSOC);
                    $art = $selArt->fetchAll();

                    $sqlArtd = "SELECT COUNT(*) FROM accounts, follows WHERE accounts.id = follows.acc_sec AND follows.acc_main = '$id'";
                    $selArtd = $con->query($sqlArtd);
                    $artd = $selArtd->fetchColumn();

                    ?>
                    <p class="label"><?= $artd; ?> Orang Diikuti</p>
                    <?php
                    ...
                    ?>
                            <div class="flow">
                                ...
                                ?>
                                    ...
                                    ?>
                                        <a href="./followings.php?un=<?= $un; ?>&acc_main=<?= $ids ?>&acc_sec=<?= $pos['acc_sec'] ?>&type=follow" class="fol">Ikuti</a>
                                    <?php
                                    ...
                                    ?>
                                    <a onclick="unfol(<?= $pos['acc_sec']; ?>)" class="ufol">Mengikuti</a>
                                <?php
                                } else if ($flod) {

                                ?>
                                    <a href="./followings.php?un=<?= $un; ?>&acc_main=<?= $ids; ?>&acc_sec=<?= $pos['acc_sec']; ?>&type=follow" class="fol">Ikuti Balik</a>
                                <?php
                                }
                    ...
                    ?>
                            <div class="fig">
                                ...
                                <div class="fol">
                                    ...
                                    ?>
                                            <a href="./followings.php?un=<?= $un; ?>&acc_main=<?= $ids ?>&acc_sec=<?= $p3['id'] ?>&type=follow">Ikuti</a>
                                        <?php
                                    ...
                                    ?>
                                        <a href="./followings.php?un=<?= $un; ?>&acc_main=<?= $ids ?>&acc_sec=<?= $p3['id'] ?>&type=follow">Ikuti Balik</a>
                                    <?php
                                    }
    ...
    <script>
        let unfol = (vlb) => {
            ...
            if (confirm('Berhenti mengikuti orang ini?')) {
                window.open("./followings.php?un=<?= $un; ?>&acc_main=<?= $ids; ?>&acc_sec=" + vlb + "&type=unfollow", '_self')
            }
